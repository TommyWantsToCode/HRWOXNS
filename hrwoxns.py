...
def usbWrite(devices, endpoint, hexData):
    ...
    print('send: ' + ' '.join(hex(ord(x)) for x in hexData))
...
def _register_commands_with_controller_state(controller_state, cli):
    ...
    async def start_hori():
        """
        start_hori - Enables hori on USB and forwards inputs
        """
        await start_hori_emulation(controller_state)

    cli.add_command(start_hori.__name__, start_hori)
